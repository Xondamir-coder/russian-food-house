<template>
	<ul class="accordion">
		<li
			...
			v-for="faq in faqs"
			:key="faq.id"
			:class="{ active: activeQuestion === faq.question }">
			<button class="accordion__item-top" @click="toggleQuestion(faq.question)">
...
const faqs = computed(() =>
	faqsStore.faqs.filter(faq => faq.category === faqsStore.activeCategory)
);

const toggleQuestion = questionTitle => {
	activeQuestion.value = activeQuestion.value === questionTitle ? '' : questionTitle;
};
</script>
